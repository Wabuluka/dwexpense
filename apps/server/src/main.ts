  ...
  cron.schedule("0 * * * *", async () => {
    ...
    try {
      ...
      for (const user of users) {
        const { createdExpenses, createdIncome } = await applyRecurringForUser(
          String(user._id),
        );
      ...
      console.log(
        `[cron] Recurring apply complete — expenses created: ${totalExpenses}, income created: ${totalIncome}`,
      );
    } catch (err) {
